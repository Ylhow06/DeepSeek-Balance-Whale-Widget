...
export default {
  ...
  apply(ctx) {
    ...
    // ========================================================================
    // 平台官方消费历史（参考 dboycht/ds-usage-export）：
    //   platform.deepseek.com/api/v0 内部接口，用平台登录态 userToken
    //   （credentials: DEEPSEEK_PLATFORM_TOKEN）抓取官方「按日精确费用」，
    //   补上小鲸鱼记账只能估算历史的短板。任意时间范围按 ≤30 天分片请求。
    // ========================================================================
    const PLATFORM_API_BASE = 'https://platform.deepseek.com/api/v0'
    const PLATFORM_TOKEN_NAME = 'DEEPSEEK_PLATFORM_TOKEN'
    const PLATFORM_MAX_WINDOW_DAYS = 30 // 平台 UI 前端限制 30 天；服务端未确认，按此分片最稳
    ...
    let platformUsageCache = null // { at, token, days }

    function bizDataOf(data) {
      if (!data || typeof data !== 'object') return null
      const d = data.data
      if (d && typeof d === 'object' && d.biz_data !== undefined) return d.biz_data
      return d
    }
    // 平台响应外壳 { code, msg, data:{ biz_code, biz_msg, biz_data } }：
    // HTTP 200 也可能是业务错误，code=40003 / authorization failed / invalid token 视为令牌失效
    function platformBizError(data) {
      const code = Number(data && data.code)
      const msg = String((data && data.msg) || '')
      const d = data && data.data
      const bizCode = d && d.biz_code
      if (code === 40003 || /authorization failed/i.test(msg) || /invalid token/i.test(msg)) {
        return '平台令牌无效或已过期（重新在 platform.deepseek.com 登录并更新 DEEPSEEK_PLATFORM_TOKEN）'
      }
      if (isFinite(code) && code !== 0) return '平台返回错误 code=' + code + ' ' + msg.slice(0, 120)
      if (bizCode !== 0 && bizCode !== null && bizCode !== undefined) {
        return '平台业务错误 biz_code=' + bizCode + ' ' + String((d && d.biz_msg) || '').slice(0, 120)
      }
      return ''
    }
    async function platformGet(path, params, token) {
      const url = PLATFORM_API_BASE + path + (params ? '?' + new URLSearchParams(params).toString() : '')
      const res = await fetch(url, {
        headers: Object.assign({ Authorization: 'Bearer ' + token }, PLATFORM_HEADERS),
        signal: AbortSignal.timeout(30000),
      })
      if (res.status === 401) throw Object.assign(new Error('平台令牌无效或已过期'), { code: 'AUTH' })
      if (res.status === 429) throw Object.assign(new Error('平台限流，请稍后再试'), { code: 'RATE', transient: true })
      // 4xx/5xx 统一读 body：平台有时用 404 外壳返回业务文案（如令牌失效/路径调整），读出来便于定位
      if (res.status === 404 || res.status >= 500) {
        let bodyTxt = ''
        try { bodyTxt = String(await res.text()).slice(0, 300) } catch (err) {}
        const e = new Error('平台接口 HTTP ' + res.status + (bodyTxt ? '：' + bodyTxt : ''))
        e.code = res.status === 404 ? 'NOT_FOUND' : 'HTTP'
        e.transient = res.status >= 500 || /限流|rate/i.test(bodyTxt)
        throw e
      }
      if (res.status !== 200) {
        // 其他 4xx（403 等）：同样读 body 文案
        let bodyTxt = ''
        try { bodyTxt = String(await res.text()).slice(0, 300) } catch (err) {}
        throw new Error('平台接口 HTTP ' + res.status + (bodyTxt ? '：' + bodyTxt : ''))
      }
      let data
      try { data = await res.json() } catch (err) { throw new Error('平台返回不是合法 JSON') }
      const bizErr = platformBizError(data)
      if (bizErr) throw Object.assign(new Error(bizErr), { code: /令牌/.test(bizErr) ? 'AUTH' : 'BIZ' })
      return bizDataOf(data)
    }
    // 日期工具：dayKey 'YYYY-MM-DD' → 平台 start/end 秒（北京时区 28800，与 UI 一致）
    function dayStartSec(dayStr) { return Math.floor(new Date(dayStr + 'T00:00:00Z').getTime() / 1000) - 28800 }
    function dayEndSec(dayStr) { return dayStartSec(dayStr) + 86400 }
    ...
    function parsePlatformRange(from, to) {
      const re = /^\d{4}-\d{2}-\d{2}$/
      let f = String(from || '').trim(), t = String(to || '').trim()
      if (!re.test(t)) t = todayKey()
      if (!re.test(f)) f = addDays(t, -29) // 缺省：最近 30 天
      if (f > t) { const x = f; f = t; t = x }
      const span = Math.round((dayStartSec(t) - dayStartSec(f)) / 86400) + 1
      if (span < 1 || span > 366) return null
      return { from: f, to: t, days: span }
    }
    // —— 平台桶时间语义（2026-09-15 实测确认，务必按此换算）——
    // 请求 `start` = 当地 00:00 的 UTC 秒（已减 tz）；单日请求返回 bucket=3600 的小时桶。
    // 桶的 time 是**绝对 UTC 秒**，而桶是按**当地整点**对齐的 —— 当地 0 点的桶，
    // 其 UTC 时刻是前一天 16:00Z。所以换算「属于哪一天/哪个小时」必须**先加 tz 再取**，
    // 直接 `toISOString()` 取 UTC 日期会把当地 0–7 点的桶算到前一天去
    // （实测 2026-09-15：正确 2.0499 vs 错误 1.5880，单日少算 0.46 元）。
    const PLATFORM_TZ = 28800
    function bucketLocalParts(tsec) {
      const d = new Date((Number(tsec) + PLATFORM_TZ) * 1000)
      const p = (n) => String(n).padStart(2, '0')
      return {
    ...
    // 单日官方口径：cost=精确费用桶（金额）、amount=token 桶（用量），两者都是小时桶。
    // 返回 { day, cost, costByModel, tokens, hours[] }，hours 供「某日各时段」使用。
    async function platformFetchOneDay(dayStr, token) {
      const start = dayStartSec(dayStr), end = dayEndSec(dayStr), tz = PLATFORM_TZ
      const [amountBiz, costBiz] = await Promise.all([
        platformGet('/usage/by_api_key/amount', { start, end, tz }, token),
        platformGet('/usage/by_api_key/cost', { start, end, tz }, token),
      ])
      const hours = new Map() // hour -> { cost, input, output, requests, models }
      function hourRec(h) {
        let r = hours.get(h)
        if (!r) { r = { cost: 0, input: 0, output: 0, requests: 0, models: {} }; hours.set(h, r) }
        return r
      }
      const costByModel = {}
      const tokens = { requests: 0, input: 0, output: 0, models: {} }
      // cost 桶 → 金额（按当地天/小时归组；单日请求理论上全落在 dayStr，仍做防御性过滤）
      if (costBiz) {
        for (const c of (Array.isArray(costBiz.data) ? costBiz.data : [])) {
          for (const s of ((c && c.series) || [])) {
            const m = s.model || '未知'
            for (const b of ((s && s.buckets) || [])) {
              const parts = bucketLocalParts(b.time)
              if (parts.day !== dayStr) continue
              const amt = Number(b.cost) || 0
              if (!amt) continue
              hourRec(parts.hour).cost += amt
              costByModel[m] = (costByModel[m] || 0) + amt
            }
          }
        }
      }
      // amount 桶 → token 数 / 请求数（同样按当地小时归组）
      if (amountBiz && Array.isArray(amountBiz.series)) {
        for (const s of amountBiz.series) {
          ...
          for (const b of ((s && s.buckets) || [])) {
            ...
            const inTok = (Number(u.PROMPT_CACHE_HIT_TOKEN) || 0) + (Number(u.PROMPT_CACHE_MISS_TOKEN) || 0)
            const outTok = Number(u.RESPONSE_TOKEN) || 0
            if (!req && !inTok && !outTok) continue
            const hr = hourRec(parts.hour)
            hr.requests += req; hr.input += inTok; hr.output += outTok
            hr.models[m] = (hr.models[m] || 0) + (inTok + outTok)
            tokens.requests += req; tokens.input += inTok; tokens.output += outTok
            tokens.models[m] = (tokens.models[m] || 0) + (inTok + outTok)
          }
        }
      }
      let cost = 0
      for (const r of hours.values()) cost += r.cost
      const hourList = Array.from(hours.keys()).sort((a, b) => a - b).map((h) => {
        const r = hours.get(h)
        return { hour: h, cost: round4(r.cost), input: r.input, output: r.output, requests: r.requests, models: r.models }
      })
      // 只保留真正有费用的模型（平台会返回全部模型键含 0 值，写进缓存会白白变大）
      const costByModelNZ = {}
      for (const m of Object.keys(costByModel)) {
        if (costByModel[m] > 0.0001) costByModelNZ[m] = round4(costByModel[m])
      }
      return { day: dayStr, cost: round4(cost), costByModel: costByModelNZ, tokens, hours: hourList }
    }
    // —— 官方记录缓存（持久化）：抓到的官方按日/按小时数据落盘，
    //    记账展示优先用它「覆盖」余额差估算，重启后也不丢 ——
    const OFFICIAL_FILE_CANDIDATES = [
      path.join(DSH_HOME, '.dshw-official.json'),
      path.join(DSH_HOME, 'profiles', 'web', '.dshw-official.json'),
    ]
    function readOfficialCache() {
      for (const p of OFFICIAL_FILE_CANDIDATES) {
        try {
    ...
    // 把抓到的按日记录并入缓存（同一天以最新一次抓取为准，覆盖旧值）
    function mergeOfficialDays(dayRecords) {
      if (!Array.isArray(dayRecords) || !dayRecords.length) return false
      const oc = readOfficialCache()
      oc.days = oc.days && typeof oc.days === 'object' ? oc.days : {}
      const now = new Date().toISOString()
      for (const d of dayRecords) {
        if (!d || !/^\d{4}-\d{2}-\d{2}$/.test(String(d.day || ''))) continue
        oc.days[d.day] = {
          cost: round4(d.cost),
          models: d.costByModel || {},
      ...
      // 保留最近 400 天，避免无限膨胀
      const keys = Object.keys(oc.days).sort()
      if (keys.length > 400) for (const k of keys.slice(0, keys.length - 400)) delete oc.days[k]
      oc.updatedAt = now
      return writeOfficialCache(oc)
    }
    function officialDayMap() {
      try {
        const oc = readOfficialCache()
        return oc.days && typeof oc.days === 'object' ? oc.days : {}
      } catch (err) { return {} }
    }

    // 任意范围抓取：逐日请求（单日才有小时桶），失败天记 failedDays；成功结果并入官方缓存。
    async function platformFetchRange(opts) {
      const range = parsePlatformRange(opts && opts.from, opts && opts.to)
      if (!range) return { ok: false, code: 'BAD_RANGE', error: '时间范围无效（1–366 天，格式 YYYY-MM-DD）' }
      let token = ''
      try {
        const cred = await ctx.credentials.resolve(PLATFORM_TOKEN_NAME)
        token = cred && cred.value ? String(cred.value) : ''
      } catch (err) {}
      if (!token) return { ok: false, code: 'NO_KEY', error: '未配置平台令牌：在模型列表里 DeepSeek 那行的「设置 → 密钥 / 接口 → 平台令牌」填入 platform.deepseek.com 的登录态 userToken 并保存（无需重启）' }
      const days = []
      const failedDays = []
      const totalCostByDay = {}, totalCostByModel = {}
      let totalTokens = { requests: 0, input: 0, output: 0 }
      let cursor = range.from
      while (true) {
        const day = cursor
        try {
          const one = await platformFetchOneDay(day, token)
          days.push(one)
          totalCostByDay[day] = round4(one.cost)
          for (const m of Object.keys(one.costByModel || {})) {
            totalCostByModel[m] = (totalCostByModel[m] || 0) + one.costByModel[m]
          }
          totalTokens.requests += Number(one.tokens && one.tokens.requests) || 0
          totalTokens.input += Number(one.tokens && one.tokens.input) || 0
          totalTokens.output += Number(one.tokens && one.tokens.output) || 0
        } catch (err) {
          failedDays.push({ day: day, error: String((err && err.message) || err).slice(0, 160), transient: !!err.transient, code: err.code || 'HTTP' })
          // 令牌失效/业务错误没必要再逐天试，直接终止
          if (err.code === 'AUTH' || err.code === 'BIZ') break
        }
        cursor = addDays(day, 1)
        if (cursor > range.to) break
      }
      // 成功的每天并入官方缓存 → 记账展示随即「覆盖」估算值
      let persisted = false
      try { persisted = mergeOfficialDays(days) } catch (err) {}
      const totalCost = round4(Object.keys(totalCostByDay).reduce((s, k) => s + totalCostByDay[k], 0))
      const totalCostByModelR = {}
      for (const m of Object.keys(totalCostByModel)) totalCostByModelR[m] = round4(totalCostByModel[m])
      return {
        ok: true,
        range: { from: range.from, to: range.to },
        totalCost, totalTokens,
        costByDay: totalCostByDay,          // { 'YYYY-MM-DD': 金额 }
        costByModel: totalCostByModelR,     // { 模型: 金额 }（仅非零）
        days: days,                         // 每日记录（含 hours 小时桶）
        failedDays,
        persisted,                          // 是否成功写入官方缓存（覆盖记账的依据）
        source: 'platform',
        currency: 'CNY',
    ...
    // 后台轻量自动同步：面板打开时顺带把最近 3 天官方值刷新一遍（10 分钟冷却，失败不重试轰炸）。
    // 目的是让「官方覆盖记账」无需手动拉取也保持最新（今天的数据尤其需要）。
    let officialAutoSyncAt = 0
    let officialAutoSyncBusy = false
    function maybeAutoSyncOfficial() {
      if (officialAutoSyncBusy) return
      const now = Date.now()
      if (now - officialAutoSyncAt < 10 * 60 * 1000) return
      officialAutoSyncAt = now
      officialAutoSyncBusy = true
      ;(async () => {
        try {
          const cred = await ctx.credentials.resolve(PLATFORM_TOKEN_NAME)
          const token = cred && cred.value ? String(cred.value) : ''
          if (!token) return
          const to = todayKey()
          const from = addDays(to, -2)
          const got = []
          for (let d = from; d <= to; d = addDays(d, 1)) {
            try { got.push(await platformFetchOneDay(d, token)) } catch (err) {
              if (err && (err.code === 'AUTH' || err.code === 'BIZ')) return
            }
          }
          if (got.length) mergeOfficialDays(got)
        } catch (err) {}
        finally { officialAutoSyncBusy = false }
      })()
    }

    // 「当天」详细数据同步：只抓今天一天（amount+cost 两个请求），
    // 拿到当天的合计/分模型/各时段小时桶后并入官方缓存。
    //
    // ⚠️ 只在**用户主动操作**时触发，绝不做后台轮询：
    //    - 点小鲸鱼手动刷新（balance.json?force=1）
    //    - 打开用量面板（usage-records.json → maybeAutoSyncOfficial）
    //    - 点「拉取历史」/「查看各时段消费」按钮
    // 自动轮询（挂件每 60s 一次 balance.json）**不拉** —— 那时用户没在看面板，
    // 拉了也看不到，纯属白打平台请求。
    // 20s 下限：连点也不会把平台打爆。
    const OFFICIAL_TODAY_SYNC_MIN_MS = 20 * 1000
    let officialTodaySyncAt = 0
    let officialTodaySyncBusy = false
    function maybeSyncTodayOfficial() {
      if (officialTodaySyncBusy) return
      const now = Date.now()
      if (now - officialTodaySyncAt < OFFICIAL_TODAY_SYNC_MIN_MS) return
      officialTodaySyncAt = now
      officialTodaySyncBusy = true
      ;(async () => {
        try {
          const cred = await ctx.credentials.resolve(PLATFORM_TOKEN_NAME)
          const token = cred && cred.value ? String(cred.value) : ''
          if (!token) return
          const one = await platformFetchOneDay(todayKey(), token)
          mergeOfficialDays([one])
          // 当天数据变了 → 让余额缓存失效，下一次 balance.json 立刻用上新的今日已用
          balanceCache = null
        } catch (err) {
          // 静默：令牌失效/限流/网络抖动都不打断刷新，下次主动操作再试
        } finally { officialTodaySyncBusy = false }
      })()
    }
    // 官方缓存里的「今天」记录（没有则 null）
    function officialTodayRecord() {
      try {
        const d = todayKey()
        const rec = officialDayMap()[d]
        return rec && isFinite(Number(rec.cost)) ? rec : null
    ...
    function usageRecordsPayload() {
      ...
      // 官方记录快照：有官方数据的日期一律用官方值「覆盖」余额差/事件估算
      let officialCache = { updatedAt: null, days: {} }
      try { officialCache = readOfficialCache() } catch (err) {}
      const official = officialCache.days && typeof officialCache.days === 'object' ? officialCache.days : {}
      function daySrc(d) { return official[d] ? 'official' : 'estimate' }
      ...
      function totalDay(d) {
        // ① 官方精确账单优先 —— 「官方消费记录覆盖记账」的落点。
        //    官方是平台结算口径（与 platform.deepseek.com/usage 一致），
        //    比余额差/事件估算更准（实测今天余额差为 0 而官方 2.05）。
        const off = official[d]
        if (off && isFinite(Number(off.cost))) return round2(off.cost)
        // ② 无官方数据 → 退回归宿估算
        if (d === today) {
          const dd = realDelta()
      ...
      function modelsFor(d) {
        // 官方有按模型拆分时直接用它（与官方合计同源，无需再对齐余额差）
        const offM = official[d]
        if (offM && offM.models && typeof offM.models === 'object' && Object.keys(offM.models).length) {
          return Object.keys(offM.models)
            .map((m) => ({ model: m, cost: round2(offM.models[m]) }))
            .filter((x) => x.cost > 0.004)
      ...
      for (let i = 0; i <= 6; i++) {
        ...
        const t = totalDay(d)
        total7 += t
        days7.push({ date: d, total: round2(t), models: modelsFor(d), src: daySrc(d) })
      }
      const daySet = {}
      Object.keys(history).forEach((d) => { if (/^\d{4}-\d{2}-\d{2}$/.test(d)) daySet[d] = 1 })
      events.forEach((e) => { if (/^\d{4}-\d{2}-\d{2}$/.test(e.day)) daySet[e.day] = 1 })
      // 官方记录里的日期也要进「全部记录」（否则只有官方数据、没有事件/归档的日子会不显示）
      Object.keys(official).forEach((d) => { if (/^\d{4}-\d{2}-\d{2}$/.test(d)) daySet[d] = 1 })
      const allDays = Object.keys(daySet).sort().reverse().map((d) => ({ date: d, total: totalDay(d), models: modelsFor(d), src: daySrc(d) }))
      const latestEvents = events.slice().sort((a, b) => b.ts - a.ts).slice(0, 500)
      const officialDayKeys = Object.keys(official)
      return {
        ok: true,
        today: { total: totalDay(today), models: modelsFor(today), src: daySrc(today) },
        days7: days7,
        total7: round2(total7),
        all: { days: allDays, events: latestEvents },
        // 官方覆盖状态（前端用于显示「官方」标记与同步时间）
        official: {
          updatedAt: officialCache.updatedAt || null,
          days: officialDayKeys.length,
          hasToday: !!official[today],
          earliest: officialDayKeys.sort()[0] || null,
        },
    ...
    async function apiSaveModel(input) {
      const p = input || {}
      // 内置模型（DeepSeek）不是注册表里的普通模型：名称/厂商/币种/接口全部固定，
      // 面板里真正能改的只有密钥。所以这里只写密钥就收工 ——
      // 否则会因为「厂商模板」下拉里本来就不列内置模板（provider 为空）而报
      // 「未知的厂商模板」，把「改个 key」变成一条死路（用户实测踩到）。
      if (String(p.id || '') === API_BUILTIN_ID) {
        const bRef = String(p.keyRef || 'DEEPSEEK_API_KEY').trim().slice(0, 64) || 'DEEPSEEK_API_KEY'
        let bSaved = false
        if (p.keyValue !== undefined && String(p.keyValue).length) {
          try {
            await ctx.credentials.set(bRef, String(p.keyValue))
            bSaved = true
          } catch (err) {}
        }
        return { ok: true, id: API_BUILTIN_ID, keySaved: bSaved, builtin: true }
      }
    ...
    async function apiModelsPayload() {
      ...
      for (const m of models) {
        ...
        if (m.id === API_BUILTIN_ID) {
          ...
          // 平台令牌（官方账单/分时段数据的数据源）：只下发"是否已配置"，
          // 值本身绝不回传前端（它是 platform.deepseek.com 的登录态 userToken）
          try {
            const pt = await ctx.credentials.resolve(PLATFORM_TOKEN_NAME)
            entry.platformTokenSet = !!(pt && pt.value)
          } catch (err) {}
          try {
            const p = await getBalance()
            if (p && p.ok) {
              entry.balance = Number(p.totalBalance)
              // ⚠ 今日已用必须与余额面板/泡泡**同源**：官方账单优先，其次余额差。
              //   这里曾直接用 ledgerTodayTotal()（只看余额差、不看官方记录），
              //   于是模型行显示 4.32 而面板合计显示官方口径 5.82，两个数对不上
              //   （用户实测报过）。getBalancePayload() 里的 todayUsageOf 就是同一套口径。
    ...
    async function getBalancePayload() {
      // 官方「今天」记录优先：刷新时已顺带拉过当日详细数据（maybeSyncTodayOfficial），
      // 有就用官方口径，和面板/各时段读同一个数；没有才退回余额差估算。
      const offToday = officialTodayRecord()
      const todayUsageOf = (led) => {
        if (offToday) return round2(offToday.cost)
        return ledgerTodayTotal(led)
      }
      const payload = await fetchBalance()
      if (!payload.ok) {
        // 余额接口失败（含限流 429）时仍附上今日已用：官方记录优先，其次事件/账本余额差
        const led = readUsageLedger()
        return {
          ...payload,
          todayUsage: todayUsageOf(led),
          todayUsageSource: offToday ? 'official' : 'estimate',
          usageMode: 'ledger',
        }
      }
      ...
      full.todayUsage = todayUsageOf(led)
      full.todayUsageSource = offToday ? 'official' : 'estimate'
    ...
    disposers.push(registerRoute({
      ...
      handler: async (req, res) => {
        try {
          // 只有「手动刷新」（点小鲸鱼 → ?force=1）才顺带拉当日详细数据。
          // 自动轮询（无 force）一律不拉：用户没在看面板，拉了也看不到，白费平台请求。
          // 非阻塞：本轮先用缓存里已有的官方值，抓回来的新值下一轮生效。
          let force = false
          try { force = new URL(req.url || '', 'http://localhost').searchParams.get('force') === '1' } catch (err) {}
          if (force) { try { maybeSyncTodayOfficial() } catch (err) {} }
    ...
    disposers.push(registerRoute({
      ...
      handler: async (req, res) => {
        try {
          ...
          // 面板打开时顺带在后台刷新最近 3 天官方值（10 分钟冷却，失败静默），
          // 让「官方覆盖记账」无需手动拉取也保持最新
          try { maybeAutoSyncOfficial() } catch (err) {}
          res.writeHead(200, JSON_HEADERS)
    ...
    // 平台官方消费历史（?from=&to= 指定范围，缺省最近 30 天；1h 内存缓存）
    disposers.push(registerRoute({
      kind: 'exact',
      path: '/dsh-whale/platform-usage.json',
      handler: async (req, res) => {
        let q
        try { q = new URL(req.url || '', 'http://localhost').searchParams } catch (err) { q = new URLSearchParams() }
        const from = String(q.get('from') || '')
        const to = String(q.get('to') || '')
        try {
          const range = parsePlatformRange(from, to)
          if (!range) {
            res.writeHead(200, JSON_HEADERS)
            res.end(JSON.stringify({ ok: false, code: 'BAD_RANGE', error: '时间范围无效（1–366 天，格式 YYYY-MM-DD）' }))
            return
          }
          const key = from + '|' + to
          const cachedNow = platformUsageCache && platformUsageCache.key === key && Date.now() - platformUsageCache.at < 3600000
          if (cachedNow) {
            res.writeHead(200, JSON_HEADERS)
            res.end(JSON.stringify(Object.assign({}, platformUsageCache.result, { cached: true })))
            return
          }
          const result = await platformFetchRange({ from, to })
          if (result.ok) platformUsageCache = { key, at: Date.now(), result }
          res.writeHead(200, JSON_HEADERS)
          res.end(JSON.stringify(result))
        } catch (err) {
          res.writeHead(200, JSON_HEADERS)
    ...
    // 某日各时段消费（?date=YYYY-MM-DD）：优先用官方缓存里的小时桶，缓存没有才去平台抓。
    disposers.push(registerRoute({
      kind: 'exact',
      path: '/dsh-whale/platform-hours.json',
      handler: async (req, res) => {
        let q
        try { q = new URL(req.url || '', 'http://localhost').searchParams } catch (err) { q = new URLSearchParams() }
        const date = String(q.get('date') || '').trim()
        const refresh = String(q.get('refresh') || '') === '1'
        try {
          if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
            res.writeHead(200, JSON_HEADERS)
            res.end(JSON.stringify({ ok: false, code: 'BAD_DATE', error: '日期格式应为 YYYY-MM-DD' }))
            return
          }
          const cached = officialDayMap()[date]
          if (cached && Array.isArray(cached.hours) && cached.hours.length && !refresh) {
            res.writeHead(200, JSON_HEADERS)
            res.end(JSON.stringify({
              ok: true, date: date, hours: cached.hours,
              cost: cached.cost, models: cached.models || {},
              tokens: cached.tokens || null, fetchedAt: cached.fetchedAt || null, cached: true,
            }))
            return
          }
          // 缓存缺失（或强制刷新）→ 现抓这一天
          ...
          if (!token) {
            // 无 token 时若缓存里有「无小时桶」的旧数据，也把已有信息返回，避免整个视图空白
            if (cached) {
              res.writeHead(200, JSON_HEADERS)
              res.end(JSON.stringify({
                ok: true, date: date, hours: cached.hours || [],
                cost: cached.cost, models: cached.models || {}, tokens: cached.tokens || null,
                fetchedAt: cached.fetchedAt || null, noKey: true,
              }))
              return
            }
            res.writeHead(200, JSON_HEADERS)
            res.end(JSON.stringify({ ok: false, code: 'NO_KEY', error: '未配置 DEEPSEEK_PLATFORM_TOKEN，无法拉取分时段数据' }))
            return
          }
          const one = await platformFetchOneDay(date, token)
          try { mergeOfficialDays([one]) } catch (err) {}
          res.writeHead(200, JSON_HEADERS)
          res.end(JSON.stringify({
            ok: true, date: date, hours: one.hours,
            cost: one.cost, models: one.costByModel, tokens: one.tokens,
            fetchedAt: new Date().toISOString(),
          }))
        } catch (err) {
          res.writeHead(200, JSON_HEADERS)
          res.end(JSON.stringify({ ok: false, code: err.code || 'ERROR', error: String((err && err.message) || err).slice(0, 200) }))
        }
      },
    }))
